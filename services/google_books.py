# services/google_books.py — FINAL: Open Library first for better covers + niche books
import httpx
...
# MASTER LOOKUP — Open Library first for better covers on niche books
async def master_lookup(isbn: str = "") -> Tuple[Optional[Dict], str]:
    result, source = await openlibrary_lookup(isbn)
    if result and result.get("title"): return result, source
